      ...
      body: ListView(
        ...
        children: [
          ...
          _card(context, 'Pipeline checklist', const [
            'Security Engine: ✓ deterministic (owns verdict)',
            'AI Provider: AVAILABLE / UNAVAILABLE (see AI STATUS above)',
            'Fallback: ACTIVE / INACTIVE (see AI STATUS above)',
            'OCR (ML Kit, on-device): ✓',
            'QR (ML Kit barcode, on-device): ✓',
            'URL Parser (parse, never visit): ✓',
            'Network Requests: NONE (no INTERNET permission)',
            'URL Fetching: DISABLED',
            'QR Execution: DISABLED',
            'History: INDICATOR-ONLY (digit-masked previews)',
            'Cloud: NOT CONFIGURED',
            'Command Center: MANUAL SYNC (clipboard JSON)',
          ]),
          ...
          _card(context, 'System architecture', const [
            'INPUT → EXTRACTION → RULE ENGINE → AI CONTEXT → '
                'FUSION → THREAT EVENT → ACTION',
